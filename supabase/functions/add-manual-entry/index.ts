...
Deno.serve(async req => {
  ...
  try {
    const { userId, ruleId, note, decidedBy }: Payload = await req.json()
    ...
    if (insertError) {
      console.error('Errore insert manual entry:', insertError)
    ...
    console.error('Errore add-manual-entry:', error)
