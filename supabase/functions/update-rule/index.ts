import { createClient } from 'npm:@supabase/supabase-js@2'

type Payload = {
  ruleId?: string
  category?: string
  label?: string
  points?: number | string
  kind?: 'bonus' | 'malus'
  boySelectable?: boolean
}

function getRuleStyle(category: string, kind: string) {
  const colorByCategory: Record<string, string> = {
    pulizia: 'sky',
    pranzo: 'orange',
    rifiuti: 'emerald',
    cucina: 'amber',
    uscite: 'cyan',
    spesa: 'cyan',
    didattica: 'violet',
    podcast: 'violet',
    laboratori: 'violet',
    cura: 'teal',
    attenzione: 'rose',
  }

  const iconByCategory: Record<string, string> = {
    pulizia: '🧹',
    pranzo: '🍽️',
    rifiuti: '♻️',
    cucina: '🍳',
    uscite: '🛒',
    spesa: '🛒',
    didattica: '🎙️',
    podcast: '🎙️',
    laboratori: '🎙️',
    cura: '👕',
    attenzione: '⚠️',
  }

  const categoryKey = category.toLowerCase()
  const matchedColor = Object.keys(colorByCategory).find(key => categoryKey.includes(key))
  const matchedIcon = Object.keys(iconByCategory).find(key => categoryKey.includes(key))

  return {
    icon: matchedIcon ? iconByCategory[matchedIcon] : (kind === 'malus' ? '⚠️' : '⭐'),
    color: matchedColor ? colorByCategory[matchedColor] : (kind === 'malus' ? 'rose' : 'orange'),
  }
}

function jsonResponse(body: unknown, status = 200) {
  return new Response(JSON.stringify(body), {
    status,
    headers: {
      'Content-Type': 'application/json',
      'Access-Control-Allow-Origin': '*',
      'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type',
      'Access-Control-Allow-Methods': 'POST, OPTIONS',
    },
  })
}

Deno.serve(async req => {
  if (req.method === 'OPTIONS') return jsonResponse({ ok: true }, 200)
  if (req.method !== 'POST') return jsonResponse({ error: 'Method not allowed' }, 405)

  try {
    const { ruleId, category, label, points, kind, boySelectable }: Payload = await req.json()

    const cleanRuleId = String(ruleId || '').trim()
    const cleanCategory = String(category || '').trim()
    const cleanLabel = String(label || '').trim()
    const rawPoints = Number(points)
    const cleanKind = String(kind || '').trim() === 'malus' ? 'malus' : 'bonus'
    const cleanBoySelectable = Boolean(boySelectable)

    if (!cleanRuleId || !cleanCategory || !cleanLabel || Number.isNaN(rawPoints)) {
      return jsonResponse({ error: 'Dati regola non validi' }, 400)
    }

    const finalPoints = cleanKind === 'malus' ? -Math.abs(rawPoints) : Math.abs(rawPoints)
    const style = getRuleStyle(cleanCategory, cleanKind)

    const supabase = createClient(
      Deno.env.get('SUPABASE_URL') || '',
      Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') || ''
    )

    const { data: updatedRule, error } = await supabase
      .from('rules')
      .update({
        category: cleanCategory,
        label: cleanLabel,
        points: finalPoints,
        kind: cleanKind,
        boy_selectable: cleanBoySelectable,
        icon: style.icon,
        color: style.color,
      })
      .eq('id', cleanRuleId)
      .select('*')
      .maybeSingle()

    if (error) {
      console.error('Errore update-rule:', error)
      return jsonResponse({ error: error.message || 'Errore modifica regola' }, 500)
    }

    if (!updatedRule) {
      return jsonResponse({ error: 'Regola non trovata' }, 404)
    }

    return jsonResponse({ ok: true, rule: updatedRule })
  } catch (error) {
    console.error('Errore update-rule catch:', error)
    return jsonResponse({ error: 'Richiesta non valida' }, 400)
  }
})