...
Deno.serve(async req => {
  ...
  try {
    ...
    const normalizedId = slugify(`${cleanCategory}-${cleanLabel}`)
    ...
    console.log('add-rule before insert', {
      id: finalId,
    ...
    if (insertError) {
      console.error('Errore add-rule insert:', insertError)
      return jsonResponse({ error: insertError.message || 'Errore aggiunta regola' }, 500)
    }

    console.log('add-rule success')
    return jsonResponse({ ok: true, ruleId: finalId })
  } catch (error) {
    console.error('Errore add-rule catch:', error)
