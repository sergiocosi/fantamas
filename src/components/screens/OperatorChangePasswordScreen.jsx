...
export default function OperatorChangePasswordScreen({
  ...
  try {
    ...
    const { data, error } = await supabase.functions.invoke('change-operator-password', {
      body: {
    ...
    if (error) {
      console.error('Errore invoke change-operator-password:', error);
      alert('Errore durante il cambio password operatore');
      return;
    }

    if (!data?.ok) {
      alert(data?.error || 'Errore durante il cambio password operatore');
      return;
    }
